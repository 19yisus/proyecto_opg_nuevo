...
<body>
        ...
        <div class="col-md-12  mt-2 py-2 mx-auto px-2">
          <div class="col-md-12 border bg-light rounded py-2 mx-auto 2 d-flex justify-content-between row">
            <div class="col-md-7 my-auto px-3  ">
              <h2 class="fw-bold text-start my-auto text-dark">Gestión de Director</h3>
            </div>
  ...
  <script>
    const app = Vue.createApp({
      ...
      methods: {
        ...
        SendData(e) {
          ...
          fetch("./Controllers/DirectorController.php", {
            method: "POST",
            body: form
        ...
        async GetData(id) {
          await fetch(`./Controllers/DirectorController.php?ope=ConsultOne&&id=${id}`)
            .then(res => res.json()).then(({
              data
            }) => {
              let director = data.director;
              this.nacionalidad = director.nacionalidad_persona;
              this.cedula = director.cedula_persona;
              this.nombre = director.nombre_persona;
              this.apellido = director.apellido_persona;
        ...
        async ChangeState(id) {
          ...
          setTimeout(async () => {
            ...
            await fetch(`./Controllers/DirectorController.php`, {
              method: "POST",
              body: form
            }).then(res => res.json()).then(result => {
              ViewAlert(result.mensaje, result.estado);
              $("#datatable").DataTable().ajax.url(`./Controllers/DirectorController.php?ope=ConsulAll&&id_periodo=${this.id_periodo}`).load();
            }).catch(error => console.error(error))
          }, 100);
        },
    ...
    $("#datatable").DataTable({
      ajax: {
        url: "./Controllers/DirectorController.php?ope=ConsulAll",
        dataSrc: "data"
      },
      columns: [{
        ...
        },
        {
          ...
          render: function(data, type, row) {
            ...
            let classStatus = row.estatus_director == 1 ? 'success' : 'danger';
            let disabled = row.estatus_director == 1 ? '' : 'disabled="disabled"';
            let btns = '';
            btns = `
                <div class="">
                  <button type="button" ${disabled} data-bs-toggle="modal" data-bs-target="#staticBackdrop" onClick="Consult(this)" data-id='${row.cedula_director}' class="btn btn-sm btn-info">
                    <i class="fa-solid fa-magnifying-glass"></i>
                  </button>
                  
                  <button type="button" onClick="CambiarEstatus(this)" data-id='${row.cedula_director}' class="btn btn-sm btn-${classStatus}">
    ...
    async function validacion(box, boxInput) {
      ...
      // if (boxInput != null && boxInput.name == "seguidirector") {
      ...
      if (cedulaValida && nombreValida && apellidoValida && radioValida) {
        app.formulario_valido = true;
